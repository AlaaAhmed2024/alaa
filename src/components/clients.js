  ...
  axios.get("http://localhost:8090/users", {
    ...
    .then(res => {
      const cities = res.data
        .map(item => item.selectedCity?.trim())
        .filter(city => city)
...
//   axios.get("http://localhost:8090/users", {
//     params: { name }  // ← نرسل اسم المستخدم للسيرفر
//   })
//     .then(res => {
//       const cities = res.data
//         .map(item => item.selectedCity?.trim())
...
//     .then(res => {
//       const cities = res.data
//         .map(item => item.selectedCity?.trim())                // نحذف الفراغات
//         .filter(city => city)                          // نحذف القيم الفارغة بعد التريم
//         .filter((city, index, self) => self.indexOf(city) === index) // نمنع التكرار
//         .map(city => ({
//           value: city,
//           label: city,
//         }));
...
// }, []);
  ...
  async function fetchData() {
    try {
      ...
      const response = await fetch(`http://localhost:8090/users?name=${name}`);
      const data = await response.json();
...
//   async function fetchData() {
//     try {
//       const name = localStorage.getItem("name"); // ⬅️ جلب اسم المستخدم من localStorage
//       if (!name) {
//         console.error("❌ لم يتم العثور على الاسم في localStorage");
//         return;
//       }
...
//       setRecordsA(data); // ⬅️ تخزين البيانات في الحالة
//     } catch (error) {
//       console.error("❌ فشل في جلب البيانات:", error);
//     }
//   }
...
const filteredRecords = recordsA.filter((row) => {
  ...
  if (searchedVal) {
    ...
    const displayedValues = [
      ...
      row.job,
      row.phone,
      row.housingSupport === "baqa" ? "باقه الدعم" :
      row.housingSupport === "no" ? "غير مدعوم" :
      row.housingSupport === "monthly" ? "قسط دعم" :
       row.housingSupport === "noMonthly" ? "قسط دعم" :
      row.housingSupport,
      row.city,

      row.realEstateBank === "alahli" ? "الاهلي":
      row.realEstateBank === "alrajhi" ? "الراجحي" :
      row.realEstateBank === "albilad" ? "البلاد":
      row.realEstateBank === "sab" ? "ساب" :
      row.realEstateBank ===  "alinma" ? "الانماء" :
      row.realEstateBank === "any" ? "الاخري" :
      row.realEstateBank,
    ...
    if (
      ...
      // !row.job.toLowerCase().includes(filter.job.toLowerCase())
         row.job.toLowerCase() !== filter.job.toLowerCase()
    ) return false;
    if (
      filter.phone &&
    ...
    //   if (
    //   filter.currentYear &&
    //   !row.currentYear.toLowerCase().includes(filter.currentYear.toLowerCase())
    // ) return false;
...
if (
  ...
  !row.currentYear?.toString().toLowerCase().includes(filter.currentYear.toLowerCase())
    ...
    //   !row.startWorkYear.toLowerCase().includes(filter.startWorkYear.toLowerCase())
    // ) return false;
  ...
  !row.startWorkYear?.toString().toLowerCase().includes(filter.startWorkYear.toLowerCase())
    ...
    //   if (
    //   filter.birthYear &&
    //   !row.birthYear.toLowerCase().includes(filter.birthYear.toLowerCase())
    // ) return false;
...
if (
  ...
  const { onDownload } = useDownloadExcel({
    ...
    filename:  ` عملاء الحسب _${new Date().toISOString().slice(0, 10)}` ,
    sheet: "العملاء",
  });
...
const [showa, setShowa] = useState(false);
  ...
  const handleEdit = async (id) => {
    ...
    setErrorMassge("    الي اعاده المحتسب للعيل رقم     " + id);
    setShowModal(true);
 ...
 // ✅ أضف الخلفية في كل صفحة
  didDrawPage: function (data) {
    ...
    if (doc.setGState) {
     ...
     doc.save(` عملاء الحسب _${new Date().toISOString().slice(0, 10)}.pdf`);
  }
...
>
  ...
  <div
    style={{
      ...
      padding: "3px 10px",
      marginBottom: "7px",
      flexWrap: "wrap", // يدعم الاستجابة للشاشات الصغيرة
    }}
  >
    <h3 style={{ margin: 0 }}>عملائي</h3>

    <div style={{ display: "flex", alignItems: "center", gap: "5px", width: "300px" }}>
      <FontAwesomeIcon
        icon={faMagnifyingGlass}
        style={{
          marginTop: "2px",
          color: darkSide ? "#000" : "#fff",
        }}
      />
      <input
        id="myInput-search"
...
>
  ...
  >
    ...
    {/* <div className="col-12 col-md-3  d-flex justify-content-md-end justify-content-center" style={{ position: "relative" }}>
      <FontAwesomeIcon
        icon={faMagnifyingGlass}
        id="search-icon"
        style={{
...
{/* <CreatableSelect
  ...
  value={
    cityOptions.find(opt => opt.value === filter.selectedCity) || {
      value: filter.selectedCity,
      label: filter.selectedCity,
    }
  }
  onChange={(selectedOption) => {
    const selectedCity = selectedOption?.value || '';
    
    // إضافة المدينة إذا كانت جديدة
    if (
      selectedCity &&
    ...
    setFilter({
      ...filter,
      selectedCity,
     // selectedDistricts: [], // تصفير الأحياء عند تغيير المدينة
    });
  }}
    formatCreateLabel={(inputValue) => inputValue}
  styles={customSelectStyle}
...
<CreatableSelect
  ...
  value={filter.selectedCities.map((city) => ({
    value: city,
    label: city,
  }))}
  onChange={(selectedOptions) => {
    ...
    // إضافة المدن الجديدة للقائمة إن لم تكن موجودة
    const newCities = selectedValues.filter(
      (city) => !cityOptions.find((c) => c.value === city)
    );
               ...
               records.map((user, i) => (
                          ...
                          <tr className="align-middle" key={i}>
                            ...
                            <td style={{ width: "120px" }}>{
                               ...
                               user.dateAdd
                                     ? moment(user.dateAdd).locale('en').format('DD/MM/YYYY HH:mm')
...
<div class="hover-container">
  ...
  <aside className={darkSide?"hover-popup shadow p-3 mb-5  rounded":"hover-popup shadow p-3 mb-5  rounded dark-arrow"} style={{
    ...
    <p>
                                    ...
                                    <p>{  "تاريخ الحسبة :" +  user.currentYear + "-" + user.currentMonth}</p>
