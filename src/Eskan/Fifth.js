...
window.logoBase64 =
  ...
  // ✅ الكفيل
  doc.setFontSize(12);
  doc.text("(الكفيل الضامن)", pageWidth / 2, y, { align: "center" });
  y += 8;
  doc.text(
    `أتعهّد وألتزم بصفتي كفيلاً متضامناً بأن أدفع لأمر شركة إسكان سلمان العقارية عند أول طلب منه لقيمة السند"`
  ...
  doc.text(`اسم الضامن: ${props.input.prsonName}`, pageWidth - 15, y, { align: "right" });
  doc.text(" : التوقيع", pageWidth - 15, y + 8, { align: "right" });


 

 const fullText2 = `هوية وطنية رقم: ${props.input.prsonId}`;
  doc.text(fullText2, 15, y, { align: "left" });
   y += 9;
   const fullWidth2 = doc.getTextWidth(fullText2);
  doc.text(`العنوان: ${props.input.prsonCity}`, 15 + fullWidth2 ,y, { align: "left" });

  // ✅ الشعار الخفيف (اختياري)
  ...
  // doc.save("سند-العميل-والكفيل.pdf");
    // ✅ الحفظ أو الطباعة
  if (mode === "print") {
    doc.autoPrint();
    window.open(doc.output("bloburl"), "_blank");
      ...
      doc.save(`سند-العميل-و الكفيل أ/${props.input.customerName}.pdf`);
  }
};
    ...
    const printPdf = () => {
      ...
      const handleDownloadPdf = async () => {
        ...
        var textName="سند لامر عميل و كفيل  "+ props.input.customerName+".pdf"
        pdf.save(textName);
      };
...
<div style={{textAlign: "center"}}>(الكفيل الضامن )   </div> 
<div>اتعهد والتزم بصفتى كفيلا متضامنا بأن ادفع لأمر شركة إسكان سلمان العقاريه عند اول طلب منه لقيمة السند في موعد استحقاقه </div>      

<div>

    <div style={{float:"right"}}>
        <div>{"اسم الضامن:"+props.input.prsonName}</div>
        <div>التوقيع :</div>
    </div>


    <div style={{float:"left"}}>
     <div>{" هوية وطنية رقم :"+props.input.prsonId}</div>
     <div>{"العنوان :"+props.input.prsonCity}</div>
    </div>
