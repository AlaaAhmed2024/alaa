...
const generateSndPdf = (props , mode = "save") => {
  ...
  const lines = [
    ...
    `بموجب هذا أتعهد أنا: ${props.input.prsonName}`,
  ];
  ...
  doc.text(`بطاقة رقم: ${props.input.prsonId}`, pageWidth - 75, y, {
    align: "right",
  });
  y += 9;

  // ✅ العنوان
  doc.text(`عنواني: ${props.input.prsonCity}`, pageWidth - 15, y, {
    align: "right" });
  y += 9;

  // ✅ الدفع
  doc.text("بأن أدفع دون قيد أو شرط لأمر: شركة إسكان سلمان العقارية", pageWidth - 15, y, { align: "right" });
  y += 9;
  ...
  otherLines.forEach((line) => {
    ...
    // ✅ اسم محرر السند (يسار)
  const fullText = `اسم محرر السند: ${props.input.prsonName}`;
  ...
  if (mode === "print") {
    ...
    window.open(doc.output("bloburl"), "_blank");
      ...
      doc.save(`سند-الكفيل أ/${props.input.customerName}.pdf`);
  }
};
    ...
    const printPdf = () => {
      ...
      const handleDownloadPdf = async () => {
        ...
        var textName="سند لامر الكفيل "+ props.input.customerName+".pdf"
        pdf.save(textName);
      };
...
{ "    إسم محرر السند:  " + props.input.prsonName }
