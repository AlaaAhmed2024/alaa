
import Button from 'react-bootstrap/Button';
import Modal from 'react-bootstrap/Modal';
import "./first.css"
import head from '../logo-head-dec.png'
import jsPDF from 'jspdf';
import html2canvas from 'html2canvas';
import React, { useRef } from 'react';



import reshape from "arabic-reshaper";
import '../offers/amiri-normal'; // لا حاجة لتصدير — التحميل يتم تلقائيًا

import 'jspdf-autotable';
import autoTable from 'jspdf-autotable';

import { amiriFont } from '../offers/amiri-normal'; // ملف الخط بصيغة base64
export default function Seventh(props) {


    if(props.input.currentBank=="alrajhi"){
    var iban="  : SA4780000388608010450452"
    var bank="الراجحي"
  }else{
        var iban="  : SA7910000025700001046608"
        var bank="الاهلي"
  }

window.logoBase64 =

      "data:image/png;base64,"




  const generateSndPdf = (props , mode = "save") => {
  const doc = new jsPDF({ orientation: "portrait", unit: "mm", format: "a4" });
  const pageWidth = doc.internal.pageSize.getWidth();
  const pageHeight = doc.internal.pageSize.getHeight();

  doc.addFileToVFS("Amiri-Regular.ttf", amiriFont);
  doc.addFont("Amiri-Regular.ttf", "Amiri", "normal");
  doc.setFont("Amiri");
  doc.setFontSize(14);

  // ✅ رأس الصفحة
  doc.setFontSize(14);
  doc.text("شركة", pageWidth - 20, 20, { align: "right" });
  doc.text("إسكان سلمان العقارية", pageWidth - 20, 30, { align: "right" });
  doc.text("س . ت : 1010657719", pageWidth - 20, 38, { align: "right" });

  doc.text("Eskan Salman", 20, 20, { align: "left" });
  doc.text("Real Estate Company", 20, 30, { align: "left" });
  doc.text("C.R: 1010657719", 20, 38, { align: "left" });

  // ✅ الشعار الواضح في رأس الصفحة (منتصف العرض) - مصغر
  if (window.logoBase64) {
    const logoW = 25;
    const logoH = 40;
    const logoX = (pageWidth - logoW) / 2;
    const logoY = 10;
    doc.addImage(window.logoBase64, "PNG", logoX, logoY, logoW, logoH);
  }

  // ✅ الشعار الخفيف (منتصف الصفحة)
  // if (window.logoBase64 && doc.setGState) {
  //   doc.setGState(new doc.GState({ opacity: 0.15 }));
  //   const imgW = 100;
  //   const imgH = 120;
  //   const centerX = (pageWidth - imgW) / 2;
  //   const centerY = (pageHeight - imgH) / 2;
  //   doc.addImage(window.logoBase64, "PNG", centerX, centerY, imgW, imgH);
  //   doc.setGState(new doc.GState({ opacity: 1 }));
  // }

  // ✅ خط أفقي واحد فوق التاريخ
  let y = 53;
  doc.line(20, y, pageWidth - 20, y);
  y += 11;

  // ✅ التاريخ
  doc.setFontSize(13);
  doc.text(`التاريخ: ${props.input.nowYearH}/${props.input.nowMonthH}/${props.input.nowDayH}`, pageWidth - 15, y, { align: "right" });
  doc.text(`Date: ${props.input.nowYear}/${props.input.nowMonth}/${props.input.nowDay}`, 15, y, { align: "left" });

  y += 10; // زيادة المسافة بعد التاريخ
 
 // ✅ العنوان الرئيسي
  doc.setFontSize(16);
  doc.text("اتفاقية إعادة الدفعة", pageWidth / 2, y, { align: "center" });

  // ✅ النص الرئيسي
  doc.setFontSize(13);
  y += 10;
  

  // const lines = [
  //   "بسم الله الرحمن الرحيم",
  //   ",,,, تحية طيبة وبعد ",
  //   ": تم الاتفاق بين كلا من ",
  //   " (1010657719) الطرف الاول: شركة اسكان سلمان العقارية سجل تجاري رقم ",
  //   `الطرف الثاني: ${props.input.ownerName}  ${props.input.typeOwner=="one"? "هويه و طنية":"سجل تجاري"} رقم )${props.input.ownerId}(`,
  //   "على اساس تتكفل شركة اسكان سلمان العقارية على ايداع الدفعة",
  //   `الى الاستاذ / ${props.input.customerName} )  المشتري( صاحب هوية وطنية )${props.input.customerId}(`,
  //   `بقيمه مبلغ (${props.number}) في حسابه لدي البنك (${bank})`,
  //   `وذلك لإتمام شراء عقار الاستاذ/ ${props.input.ownerName}`,
  //   `علما بان قيمه العقار الفعلي مبلغ )${props.input.price}()`,
  //   `وسوف يتم اصدار شيك مصدق من البنك بقيمة )${props.input.cheq}( باسم الطرف الثاني`,
  //   "وبناء عليه فان الطرف الثاني ملزم بعد استلام الشيكات من البنك",
  //   `باعادة مبلغ و هو ${props.number} الى شركة إسكان سلمان العقارية`,
  //   ` ${iban} و هو ${ bank}  : عن طريق التحويل على حساب الشركة لدي بنك `,
  //   ".......وهذا ما تم الاتفاق عليه والله ولي التوفيق "

  // ];



  const lines = [
  "بسم الله الرحمن الرحيم",
  "،،،، تحية طيبة وبعد",
  ": تم الاتفاق بين كلا من",
  "(الطرف الاول: شركة اسكان سلمان العقارية سجل تجاري رقم )1010657719",
  `الطرف الثاني: ${props.input.ownerName}  ${props.input.typeOwner == "one" ? "هوية وطنية" : "سجل تجاري"} رقم  ) ${props.input.ownerId} (`,
  "على أساس تتكفل شركة اسكان سلمان العقارية على ايداع الدفعة",
  `إلى الأستاذ / ${props.input.customerName} المشتري (  صاحب هوية وطنية  ) ) ${props.input.customerId} (`,
  `بقيمة مبلغ ) ${props.number} ( في حسابه لدى البنك  ${bank}  `,
  `وذلك لإتمام شراء عقار الأستاذ / ${props.input.ownerName}`,
  `علماً بأن قيمة العقار الفعلي مبلغ ) ${props.input.price} (`,
  `وسوف يتم إصدار شيك مصدق من البنك بقيمة ) ${props.input.cheq} ( باسم الطرف الثاني`,
  "وبناء عليه فإن الطرف الثاني ملزم بعد استلام الشيكات من البنك",
  `بإعادة مبلغ وقدره ) ${props.number} ( إلى شركة إسكان سلمان العقارية`,
  `  ( عن طريق التحويل على حساب الشركة لدى بنك وهو  ${iban}  : ) ${bank} `,
  "....... وهذا ما تم الاتفاق عليه والله ولي التوفيق "
];




  lines.forEach((line) => {
    doc.text(line, pageWidth - 15, y, { align: "right" });
    y += 9;
  });




  // // ✅ التوقيع والختم
  // y += 10;
  // doc.text("توقيع البائع", pageWidth - 50, y);
  // doc.text("ختم الشركة", 50, y);
  //  y += 6;
  //  if(props.input.ownerPrson=="no"){

  //  }else {
  //  doc.text( ` بموجب وكاله رقم  : ${props.input.ownerAdd}` , pageWidth - 50, y);
  //   y += 6;
  //  doc.text( `   الوكيل  : ${props.input.ownerNameAdd}` , pageWidth - 50, y);
  //    y += 6;
  //  doc.text( `   هويه الوكيل  : ${props.input.ownerIdAdd}` , pageWidth - 50, y);

  //  }



  // ✅ التوقيع والختم
y += 9;
doc.text("توقيع البائع", pageWidth - 50, y); // يمين
doc.text("ختم الشركة", 50, y); // يسار

// ✅ بيانات الوكيل (بشرط وجودها)
if (props.input.ownerPrson !== "no") {
  y += 8;
  const marginRight = pageWidth - 20; // أقرب إلى الداخل
  doc.text(`بموجب وكالة رقم: ${props.input.ownerAdd}`, marginRight, y, { align: "right" });
  y += 8;
  doc.text(`الوكيل: ${props.input.ownerNameAdd}`, marginRight, y, { align: "right" });
  y += 8;
  doc.text(`هوية الوكيل: ${props.input.ownerIdAdd}`, marginRight, y, { align: "right" });
}


  // ✅ حفظ
  // doc.save("اتفاقية-إعادة-الدفعة.pdf");
    // ✅ الحفظ أو الطباعة
  if (mode === "print") {
    doc.autoPrint();
    window.open(doc.output("bloburl"), "_blank");
  } else {
   doc.save(` اتفاقية-إعادة-الدفعه أ/ ${props.input.customerName}.pdf`);
  }
};














    const printPdf = () => {
        // window.print();
      };

            const pageRef = useRef();
      const handleDownloadPdf = async () => {
        const input = pageRef.current;
    
        const canvas = await html2canvas(input, { windowHeight: 1000 , scale:2});
        const imgData = canvas.toDataURL('image/png');
    
        const pdf = new jsPDF();
        const imgProps = pdf.getImageProperties(imgData);
        const pdfWidth = pdf.internal.pageSize.getWidth();
        const pdfHeight = (imgProps.height * pdfWidth) / imgProps.width;

    
        pdf.addImage(imgData, 'PNG', 0, 0, pdfWidth, pdfHeight);
    
        // هنا تكتب الاسم اللي تبغاه للملف
     
        var textName="اتفاقية إعادة الدفعة  " + props.input.customerName+".pdf"
        pdf.save(textName);
      };



      

  return (
    <Modal
      {...props}
      size="lg"
      aria-labelledby="contained-modal-title-vcenter"
      centered
      fullscreen={true}
     

      className="zindex f-sc-show"

    >
      <Modal.Header closeButton>
        <Modal.Title id="contained-modal-title-vcenter">
        اتفاقية إعادة الدفعة
        </Modal.Title>
      </Modal.Header>
      <Modal.Body>
       
        <div class="book all">
    <div class="page" ref={pageRef} style={{ direction: 'rtl', textAlign: 'right'}}>
        <div class="subpage eskan-salman">
          <div style={{display:"flex" , justifyContent:"space-between"}}>
            <div style={{fontSize:"22px"}}>
              <p>شركة </p>
              <p> اسكان سلمان العقارية</p>
              <p>س . ت : 1010657719 </p>
            </div>
            <div>

          <img  style={{height: "140px"}} alt="eskan" src={head}/>
            </div>
            <div style={{textAlign:"left" , fontSize:"22px"}}> 
               <p>Eskan Salman</p>
               <p>Real Estate Company</p>
               <p>C.R: 1010657719</p>
            </div>

          </div>
      <hr></hr>
    
      <div>
            <p style={{    float: "right"}}> { " التاريخ    :  " + props.input.nowYearH+"/"+ props.input.nowMonthH+"/"+props.input.nowDayH}</p>
            <p style={{   
               width: "180px"
             ,   float: "left"}}> { "  Date   :  " + props.input.nowYear+"/"+ props.input.nowMonth+"/"+props.input.nowDay }</p>
             <p style={{textAlign: "center",clear: "both"}}>  اتفاقية إعادة الدفعة </p>
            </div>
            <div style={{clear: "both"}}>





      


          <div style={{fontSize:"16px"}}>


<div>
        <div>

        بسم الله الرحمن الرحيم              
              
              

       
        </div> 

      <div>
      تحية طيبة وبعد ,,,,
      </div>


      <div> 
      تم الاتفاق بين كلا من : 
      </div>
      <div>
    

      الطرف الاول: شركة اسكان سلمان العقارية         سجل تجاري رقم (1010657719)
  
      </div> 

      <div> 
      <div>

        {"   الطرف الثاني: "+props.input.ownerName+"   هويه وطنية رقم ("+props.input.ownerId+")"}
   

      </div>

</div>

<div> 
علي اساس تتكفل شركة اسكان سلمان العقارية على ايداع الدفعة
    
     </div>











<div>
{"الي الاستاذ /"+props.input.customerName +" ( المشتري) صاحب هوية وطنية ("+ props.input.customerId +")"}
  
    </div>
<div>

    {"بقيمه مبلغ ("+props.number+")   في حسابه لدي البنك  ("+ bank +")"}

 
        </div>
<div>
    {"وذلك لإتمام شراء عقار الاستاذ/ "+props.input.ownerName}


    </div>
<div>


{"علما بان قيمه العقار الفعلي مبلغ ("+props.input.price+")"}
<div>


{"وسوف يتم اصدار شيك مصدق من البنك بقيمة ("+props.input.cheq+") باسم الطرف الثاني"}
</div>

<div>
وبناء عليه فان الطرف الثاني ملزم بعد استلام الشيكات من البنك 
</div>
<div>

    {"باعادة مبلغ و هو   "+"  " + props.number+ " " +"الى شركة إسكان سلمان العقارية "}



   </div>
   <div>

    {" عن طريق التحويل على حساب الشركة لدي بنك   :" +bank+ "  و هو "+iban}

   </div>


   <div style={{marginBottom: "20px"}}>
   وهذا ما تم الاتفاق عليه والله ولي التوفيق .......

    
    
    </div>

   <div>
     <div style={{    float: "right" , marginRight:"80px"}}>
     <div>توقيع البائع   </div>
  </div>


     <div style={{    float: "left" , marginLeft:"80px"}}>
       <div>   ختم الشركة  </div>
     </div>

</div>
      
</div></div>     
        

                                   
      





</div>
            
            
            
            </div>  
            </div>    
    </div>
  
</div>
      </Modal.Body>
      <Modal.Footer>
        <Button variant="outline-danger" onClick={props.onHide}>Close</Button>
         {/* <Button variant="outline-secondary"  onClick={printPdf}>طباعة</Button>
         <Button onClick={handleDownloadPdf}> تحميل </Button> */}
                    <Button variant="outline-secondary" onClick={() => generateSndPdf(props, "print")}>
    طباعة 
  </Button>

  <Button variant="outline-success" onClick={() => generateSndPdf(props, "save")}>
    تحميل PDF
  </Button>
      </Modal.Footer>
    </Modal>
  );
}







