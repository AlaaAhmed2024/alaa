...
faObjectGroup,
  ...
  faComment,
  faHourglassStart,
  faCalendarAlt,           // للسطح مثلاً أو الطوابق
...
import { validateOfferInputs } from "./validationUtils";
...
export default function AddOffersDetails(props) {
  const arrData = props.editClint;

  // const minAllowedDate = dayjs("1925-01-01"); // أقل تاريخ
  // const maxAllowedDate = dayjs("2008-12-31"); // أعلى تاريخ
...
const [uploadedImageUrl, setUploadedImageUrl] = useState(""); // هذا هو الذي ترسله إلى قاعدة البيانات
...
const [userDataEdit, setUSerDataEdit] = useState();
  ...
  //    { value: "أحمد", label: "أحمد" },
  //    { value: "سارة", label: "سارة" },
 ...
 const [agentNameOptions, setAgentNameOptions] = useState([]);
  ...
  axios.get("http://localhost:8090/offersAll")
    .then((response) => {
      ...
      // دالة لإرجاع قائمة خيارات فريدة من حقل معين
      ...
      setEmployeeOptions(getUniqueOptions("selectedName"));   // الموظف أو اللي أضاف العرض
      setOwnerNameOptions(getUniqueOptions("ownerName"));     // اسم المالك
      setCompanyNameOptions(getUniqueOptions("companyName")); // اسم الشركة
      setAgentNameOptions(getUniqueOptions("agentName"));     // اسم الوسيط
    })
    .catch((error) => {
  ...
  const handleChange = async (selectedOption) => {
    if (!selectedOption) {
      setcalulationInputs({ ...calulationInputs, selectedName: "" });
      return;
    }
    ...
    if (!exists) {
      try {
        await axios.post("http://localhost:8090/offersAll", {
  ...
  // عند الاختيار أو الإضافة - المالك
const handleChangeOwner = async (selectedOption) => {
  if (!selectedOption) {
    setcalulationInputs({ ...calulationInputs, ownerName: "" });
    return;
  }

  const exists = ownerNameOptions.find(
    (opt) => opt.value === selectedOption.value
  );
  ...
  if (!exists) {
    try {
      await axios.post("http://localhost:8090/offersAll", {
        ownerName: selectedOption.value,
      });
      console.log("تم حفظ اسم المالك الجديد بنجاح ✅");
    } catch (err) {
      console.error("فشل حفظ اسم المالك ❌:", err);
    }

    setOwnerNameOptions([...ownerNameOptions, selectedOption]);
  }
...
// عند الاختيار أو الإضافة - المؤسسة
const handleChangeCompany = async (selectedOption) => {
  if (!selectedOption) {
    setcalulationInputs({ ...calulationInputs, companyName: "" });
    return;
  }

  const exists = companyNameOptions.find(
    (opt) => opt.value === selectedOption.value
  );
  ...
  if (!exists) {
    try {
      ...
      console.log("تم حفظ اسم المؤسسة الجديد بنجاح ✅");
    } catch (err) {
      console.error("فشل حفظ اسم المؤسسة ❌:", err);
    }

    setCompanyNameOptions([...companyNameOptions, selectedOption]);
  }
...
// عند الاختيار أو الإضافة - 
const handleChangeAgentName = async (selectedOption) => {
  if (!selectedOption) {
    setcalulationInputs({ ...calulationInputs, agentName: "" });
    return;
  }

  const exists = agentNameOptions.find(
    (opt) => opt.value === selectedOption.value
  );
  ...
  if (!exists) {
    try {
      await axios.post("http://localhost:8090/offersAll", {
        agentName: selectedOption.value,
      });
      console.log("تم حفظ اسم المؤسسة الجديد بنجاح ✅");
    } catch (err) {
      console.error("فشل حفظ اسم المؤسسة ❌:", err);
    ...
    setAgentNameOptions([...agentNameOptions, selectedOption]);
  }
...
//     var editAqarFloor = arrData.aqarFloor;
//     var editAqarStairs = arrData.aqarStairs;
//     var editAqarConnected = arrData.aqarConnected	;
//     var editAqarType = arrData.aqarType;
//     var editArea = arrData.area;
//     var editPrice = arrData.price;
   
//     var editComments = arrData.comments;
//     var editLink = arrData.link;
//     var EditselectedName = arrData.selectedName;
//    var EditselectedDistricts=    arrData.selectedDistricts;
//    var EditselectedCity=    arrData.selectedCity;
//    var EditselectedRegion=     arrData.selectedRegion;
//     var editLocationCoordinates=arrData.locationCoordinates;
//   }
  ...
  //   locationCoordinates:editLocationCoordinates,
  // });
...
if (arrData === undefined) {
  ...
  // تاب 2 - تفاصيل العقار
  var editRoomsCountMastar="";
  var editRoomsCount = "";
  var editBathroomsCount = "";
  var editMajlesCount = "";
  var editHallsCount = "";
  var editMaqlatCount="";

  var editKitchenCount = "";
  var editAqarFacade = [];

  // عرض الشوارع لكل واجهة (streetsWidth) كائن فارغ
  var editStreetsWidth = {};

  // تاب 3 - مميزات العقار
  var editFeatureCarEntrance = false;
  var editFeatureYard = false;
  var editFeatureStorage = false;
  var editFeatureAnnex = false;
  var editFeatureMaid = false;
  var editFeatureGuard = false;
  var editFeatureDriver = false;
  var editFeatureElevator = false;
   var editFeatureElevatorT = false;

var editFeatureRoof = false;
var editFeatureNearServices = false;
var editFeatureLaundryRoom = false;
var editFeatureBalcony = false;
var editFeatureParking = false;

var editStatusOffer="انتظار العقد"


  var editcontractNumber= "";
  var editownerType= "";
  var editplotNumber= "";
  var editplanNumber= "";
  var editcontractDuration= "";
  var edithasAgency= "";
  var editownerName= "";
  var editcompanyName= "";
  var editcompanyRegistry= "";
  var editownerID= "";
  var editownerPhone= "";
  var editownerBirthDate= "";
  var editagencyNumber= "";
  var editagentName= "";
  var editagentID= "";
  var editagentPhone= "";
  var editagentBirthDate= "";
var   editTypeOffers="yesOffers";
  ...
  var EditselectedCity = arrData.selectedCity;
  var EditselectedRegion = arrData.selectedRegion;
   var editAqarState=arrData.aqarState;
  var editLocationCoordinates = arrData.locationCoordinates;

  
  // تاب 2 - تفاصيل العقار
  var editRoomsCountMastar=arrData.roomsCountMastar;
  var editRoomsCount = arrData.roomsCount;
  var editBathroomsCount = arrData.bathroomsCount;
  var editMajlesCount = arrData.majlesCount;
  var editHallsCount = arrData.hallsCount;
   var editMaqlatCount=arrData.maqlatCount;
  var editKitchenCount = arrData.kitchenCount;



  // تحويل النص المفصول بفواصل إلى مصفوفة
  var editAqarFacade = typeof arrData.aqarFacade === "string" && arrData.aqarFacade.length > 0
    ? arrData.aqarFacade.split(",").map(s => s.trim())
    : Array.isArray(arrData.aqarFacade)
    ? arrData.aqarFacade
    : [];

  // عرض الشوارع لكل واجهة (streetsWidth) - فك JSON أو تعيين فارغ
  var editStreetsWidth = typeof arrData.streetsWidth === "string" && arrData.streetsWidth.length > 0
    ? JSON.parse(arrData.streetsWidth)
    : {};

  // تاب 3 - مميزات العقار
  var editFeatureCarEntrance = !!arrData.featureCarEntrance;
  var editFeatureYard = !!arrData.featureYard;
  var editFeatureStorage = !!arrData.featureStorage;
  var editFeatureAnnex = !!arrData.featureAnnex;
  var editFeatureMaid = !!arrData.featureMaid;
  var editFeatureGuard = !!arrData.featureGuard;
  var editFeatureDriver = !!arrData.featureDriver;
      var editFeatureElevator = !!arrData.featureElevator;
      var editFeatureElevatorT = !!arrData.featureElevatorT;

     var editFeatureRoof = !!arrData.featureRoof;
     var editFeatureNearServices = !!arrData.featureNearServices;
   var editFeatureLaundryRoom = !!arrData.featureLaundryRoom;
   var editFeatureBalcony = !!arrData.featureBalcony;
   var editFeatureParking = !!arrData.featureParking;

  var editStatusOffer=arrData.statusOffer;



  
  var editcontractNumber=arrData.contractNumber;
  var editownerType=arrData.ownerType;
  var editplotNumber=arrData.plotNumber;
  var editplanNumber=arrData.planNumber;
  var editcontractDuration=arrData.contractDuration;
  var edithasAgency=arrData.hasAgency;
  var editownerName=arrData.ownerName;
  var editcompanyName=arrData.companyName;
  var editcompanyRegistry=arrData.companyRegistry;
  var editownerID=arrData.ownerID;
  var editownerPhone=arrData.ownerPhone;
  var editownerBirthDate=arrData.ownerBirthDate;
  var editagencyNumber=arrData.agencyNumber;
  var editagentName=arrData.agentName;
  var editagentID=arrData.agentID;
  var editagentPhone=arrData.agentPhone;
  var editagentBirthDate=arrData.agentBirthDate;

  var   editTypeOffers=arrData.typeOffers;
}
...
const [calulationInputs, setcalulationInputs] = useState({
  ...
  selectedCity: EditselectedCity,
  selectedRegion: EditselectedRegion,
  aqarState:editAqarState,
  locationCoordinates: editLocationCoordinates,

  // تاب 2
  roomsCountMastar :editRoomsCountMastar,
  roomsCount: editRoomsCount,
  bathroomsCount: editBathroomsCount,
  majlesCount: editMajlesCount,
  hallsCount: editHallsCount,
  maqlatCount:editMaqlatCount,
  kitchenCount: editKitchenCount,
  aqarFacade: editAqarFacade,

  streetsWidth: editStreetsWidth, // عرض الشوارع لكل واجهة

  // تاب 3
  featureCarEntrance: editFeatureCarEntrance,
  featureYard: editFeatureYard,
  featureStorage: editFeatureStorage,
  featureAnnex: editFeatureAnnex,
  featureMaid: editFeatureMaid,
  featureGuard: editFeatureGuard,
  featureDriver: editFeatureDriver,
  featureElevator:editFeatureElevator ,
  featureElevatorT:editFeatureElevatorT,


  featureRoof: editFeatureRoof,
  featureNearServices: editFeatureNearServices,
  featureLaundryRoom: editFeatureLaundryRoom,
  featureBalcony: editFeatureBalcony,
  featureParking: editFeatureParking,

  statusOffer:editStatusOffer,

   contractNumber: editcontractNumber ,
  ownerType:  editownerType,
  plotNumber:  editplotNumber,
  planNumber:  editplanNumber,
  contractDuration:  editcontractDuration,
  hasAgency:  edithasAgency,
  ownerName:  editownerName,
  companyName:  editcompanyName,
  companyRegistry:  editcompanyRegistry,
  ownerID:  editownerID,
  ownerPhone:  editownerPhone,
  ownerBirthDate:  editownerBirthDate,
  agencyNumber:  editagencyNumber,
  agentName:  editagentName,
  agentID:  editagentID,
  agentPhone:  editagentPhone,
  agentBirthDate:  editagentBirthDate,
  typeOffers:editTypeOffers,
});
...
//    useEffect(() => {
//   if (arrData) {
//     setPreviewImage(arrData.image || null);
//     setUploadedImageUrl(arrData.image || "");
//   }
// }, [arrData]);
...
useEffect(() => {
  if (arrData) {
    ...
    setPreviewImages({
      ...
      imageFacade: arrData.imageFacade || null,
      imageBathroom: arrData.imageBathroom || null,
      imageKitchen: arrData.imageKitchen || null,
      imageRoom: arrData.imageRoom || null,
      imageLiving: arrData.imageLiving || null,


        imageAgent: arrData.imageAgent || null,
          imageContract: arrData.imageContract || null,
    });
...
//         // اضبط حجم canvas ليكون مثل الصورة الأصلية
...
//         logo.src = logoBase64; // تأكد من تعريف logoBase64

//         logo.onload = function () {
//           // تحديد حجم وشكل اللوجو (مستطيل بنسبة 2:1)
//           const logoWidth = canvas.width * 0.5;  // 50% من عرض الصورة الأصلية
//           const logoHeight = logoWidth / 2;      // نصف العرض للارتفاع (نسبة 2:1)
//           const x = (canvas.width - logoWidth) / 2;
//           const y = (canvas.height - logoHeight) / 2;

//           // اجعل اللوجو شفافًا جزئيًا
//           ctx.globalAlpha = 0.7;
//           ctx.drawImage(logo, x, y, logoWidth, logoHeight);
//           ctx.globalAlpha = 1.0;

//           // احصل على البيانات الجديدة كصورة
//           const mergedImage = canvas.toDataURL("image/png");

//           // حفظ للصورة المعروضة والمرفوعة
//           setSelectedImage(dataURLtoFile(mergedImage, file.name));
...
//           // 🟡 كتابة الرقم (nextId) تحت اللوجو
//           const text = `#${nextId}`; // ← الرقم الجديد مثل #13
...
//           const textY = y + logoHeight + 10; // أسفل اللوجو بـ 10 بكسل
...
const handleImageGroupChange = (e, key) => {
  const file = e.target.files[0];
  if (!file) return;
    ...
    img.onload = function () {
      ...
      logo.onload = function () {
        ...
        setSelectedImages((prev) => ({
          ...prev,
          [key]: dataURLtoFile(mergedImage, file.name),
        }));
  ...
  axios.get("http://localhost:8090/offersAll")
    .then((res) => {
      ...
      // استخراج المدن المرتبطة بالمنطقة المحددة
      if (calulationInputs.selectedRegion) {
        const cities = data
          .filter((item) => item.selectedRegion === calulationInputs.selectedRegion)
      ...
      if (calulationInputs.selectedCity) {
        const districts = data
          .filter((item) => item.selectedCity === calulationInputs.selectedCity)
...
}, [calulationInputs.selectedRegion, calulationInputs.selectedCity]);
...
// const cityOptions = calulationInputs.selectedRegion
//   ? Object.keys(regionsData[calulationInputs.selectedRegion].cities).map((city) => ({
//       value: city,
//       label: city,
//     }))
//   : [];
    ...
    reader.onload = function (event) {
      ...
      img.onload = function () {
        ...
        logo.onload = function () {
          ...
          // 🟡 كتابة الرقم تحت اللوجو مع حدود لتوضيح النص
          const text = `#${nextId}`;
          const fontSize = canvas.width * 0.045;
          ctx.font = `${fontSize}px Arial`;
          ctx.textAlign = "center";
...
useEffect(() => {
  if (calulationInputs.selectedRegion && calulationInputs.selectedCity) {
    const newDistricts =
      regionsData[calulationInputs.selectedRegion].cities[calulationInputs.selectedCity] || [];
...
const onSubmitChangeEdit = async (ide) => {
 ...
 const errors = validateOfferInputs({
  ...calulationInputs,
  image: selectedImages.image, // ← أضف هذا الجزء
      imageContract:selectedImages.imageContract,
    imageAgent:selectedImages.imageAgent,
});
  ...
  // ✅ إذا لا يوجد أخطاء، تابع الإرسال
    ...
    setErrorMassge("تم تحديث العرض" + ide);
  setShowa(true);
  setShowModal(true);
  ...
  for (let key in selectedImages) {
    ...
    if (file) {
      ...
      const cloudData = await res.json();
      if (cloudData.secure_url) {
        uploaded[key] = cloudData.secure_url;
      } else {
        throw new Error(`فشل رفع صورة ${key}`);
      }
    }
  }
  return uploaded;
    ...
    // تجهيز قيم aqarFacade كنص مفصول بفواصل
    const facadeValues = calulationInputs.aqarFacade
      ? calulationInputs.aqarFacade.map((item) => (typeof item === "string" ? item : item.value || "")).filter(Boolean)
      : [];

    // تأكد من تحويل streetsWidth إلى نص JSON قبل الإرسال إذا موجود
    const streetsWidthStr = calulationInputs.streetsWidth
      ? JSON.stringify(calulationInputs.streetsWidth)
      : null;
    ...
    const updatedData = {
      ...calulationInputs,
      aqarFacade: facadeValues.join(","),
      // image: imageUrl,
      image:uploadedImages.image ,
      streetsWidth: streetsWidthStr,





    imageFacade: uploadedImages.imageFacade,
  imageBathroom: uploadedImages.imageBathroom,
  imageKitchen: uploadedImages.imageKitchen,
  imageRoom: uploadedImages.imageRoom,
  imageLiving: uploadedImages.imageLiving,

    imageAgent: uploadedImages.imageAgent,
      imageContract: uploadedImages.imageContract,
...
//     // إذا تم اختيار صورة جديدة، ارفعها أولًا
//     if (selectedImage) {
//       const data = new FormData();
//       data.append("file", selectedImage);
//       data.append("upload_preset", "default_preset"); // ← غيّرها
   

//       const res = await fetch(
//         "https://api.cloudinary.com/v1_1/dexc22lcl/image/upload",
//         {
//           method: "POST",
//           body: data,
//         }
...
//         setUploadedImageUrl(imageUrl);
//       } else {
//         console.error("فشل رفع الصورة:", cloudData);
//         setErrorMassge("فشل في رفع الصورة، يرجى المحاولة لاحقًا.");
//         setShowa(true);
//         return;
//       }
//     }
...
//     const requestData = {
//       ...calulationInputs,
//       image: imageUrl, // ← أضف رابط الصورة
//     };

//     // أرسل البيانات إلى السيرفر
//     await axios.post("http://localhost:8090/createOffer", requestData);
...
//     setErrorMassge("فشل في الإرسال، حاول لاحقًا");
//     setShowa(true);
//   }
// };
...
//   setErrorMassge("تم الإضافة إلى العملاء");
//   setShowa(true);
//   setShowModal(true);
...
//     // ✅ تجهيز قيم الواجهات (aqarFacade) وتحويلها إلى نص مفصول بفواصل
//     const facadeValues = Array.isArray(calulationInputs.aqarFacade)
//       ? calulationInputs.aqarFacade.map((item) =>
//           typeof item === "string" ? item : item.value || ""
//         ).filter(Boolean)
//       : [];

//     // ✅ تجهيز عرض الشوارع (streetsWidth)
//     const streetsWidth = calulationInputs.streetsWidth || {};

//     // ✅ تجهيز البيانات للإرسال إلى السيرفر
//     const requestData = {
//       ...calulationInputs,
//       aqarFacade: facadeValues.join(","), // ← حفظ الواجهات كنص
//       streetsWidth: JSON.stringify(streetsWidth), // ← حفظ عرض الشوارع كـ JSON نصي
//       image: imageUrl,
//     };

//     await axios.post("http://localhost:8090/createOffer", requestData);

//     // ✅ إعادة ضبط حالة الصورة والمعاينة
//     setPreviewImage(null);
//     setSelectedImage(null);
//     setUploadedImageUrl("");
//   } catch (err) {
...
//       ...calulationInputs,
//       aqarFacade: facadeValues.join(","),
//       streetsWidth: JSON.stringify(streetsWidth),
//       image: imageUrl,
//     };

//     await axios.post("http://localhost:8090/createOffer", requestData);

//     // ✅ التوجيه إلى صفحة العروض
//     window.location.href = "/#/show-offers";
//   } catch (err) {
//     console.error("حدث خطأ أثناء الإرسال:", err);
//     setErrorMassge("فشل في الإرسال، حاول لاحقًا");
//     setShowa(true);
...
const errors = validateOfferInputs({
  ...
  return uploaded;
    ...
    //     setUploadedImageUrl(imageUrl);
    //   } else {
    //     setErrorMassge("فشل رفع الصورة، حاول لاحقًا.");
    //     setShowa(true);
    //     setLoading(false);
    //     return;
    //   }
    ...
    // تجهيز عرض الشوارع
    const streetsWidth = calulationInputs.streetsWidth || {};
    const streetsWidthStr = JSON.stringify(streetsWidth);
   const uploadedImages = await uploadImagesToCloudinary();
...
console.log("📤 البيانات المرسلة:", requestData);
    ...
    setShowa(true);
    setShowModal(true);
     window.location.href = "/alaa/#/show-offers";
  ...
  const btnIsDisableToAdd =
   ...
   calulationInputs.locationCoordinates == ""||
    calulationInputs.aqarState=="no"
 ...
 if (loading) {
    return <AddOffersDetails />;
  }
 


  // const handleChange = (selectedOption) => {
  //   // إذا أدخل اسم غير موجود، سيتم إنشاؤه
  //   if (!employeeOptions.find((opt) => opt.value === selectedOption.value)) {
  //     setEmployeeOptions([...employeeOptions, selectedOption]);
  //   }
  ...
  return (
    <div div style={{ marginTop: "10px", height: "100vh" }}>
                ...
                addNote={"تمت الاضافه الي العروض  "}
  ...
  <Container style={{ marginTop: "20px" , marginBottom: "100px" }} className={darkSide?" labels-modal-color ba-img-light" :" labels-modal-color-dark ba-img-dark dark-lib"}>
      <Tabs defaultActiveKey="tab1" id="property-tabs"  className="tab-d-l" style={{marginBottom:"20px" , padding:"4px" , backgroundColor:"#d7e0ff"}}>
        ...
        <Tab eventKey="tab1" title="البيانات الاساسية">
      ...
      >
        <Row className="row-700 " >
          <Col xs={12} md={4}>
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
             ...
             {calulationInputs.aqarType=="فيلا"?(
              ...
              </>): calulationInputs.aqarType=="دور مع ملحق"||calulationInputs.aqarType=="فيلا روف"||calulationInputs.aqarType=="فيلا تاون هاوس"||calulationInputs.aqarType== "شقه روف" ? (
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <CreatableSelect
                ...
                value={employeeOptions.find(
                  (e) => e.value === calulationInputs.selectedName
                )}
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <Select
                ...
                value={regionOptions.find(
                  (r) => r.value === calulationInputs.selectedRegion
                )}
          ...
          <Col xs={12} md={4}>
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <CreatableSelect
                ...
                options={cityOptions}
                value={
                  cityOptions.find((c) => c.value === calulationInputs.selectedCity) ||
                  (calulationInputs.selectedCity && {
                ...
                onChange={(selectedOption) => {
                  const exists = cityOptions.find(
                    (opt) => opt.value === selectedOption.value
                  );
                  if (!exists) {
                    setCityOptions([...cityOptions, selectedOption]);
                    // يمكنك أيضًا إرسالها إلى السيرفر هنا باستخدام POST
                  }
                  setcalulationInputs({
                    ...calulationInputs,
                ...
                isDisabled={!calulationInputs.selectedRegion}
                formatCreateLabel={(inputValue) => inputValue}
                styles={customSelectStyle}
              />
              <span className="underline-input"></span>
              ...
              <CreatableSelect
                ...
                value={
                  districtOptions.find(
                    (d) => d.value === calulationInputs.selectedDistricts
                  ) ||
                  (calulationInputs.selectedDistricts && {
                    value: calulationInputs.selectedDistricts,
                    label: calulationInputs.selectedDistricts,
                  })
                }
                onChange={(selectedOption) => {
                  const exists = districtOptions.find(
                    (opt) => opt.value === selectedOption.value
                  );
                  if (!exists) {
                  ...
                  setcalulationInputs({
                    ...calulationInputs,
                    selectedDistricts: selectedOption.value,
                  });
                }}
                isDisabled={!calulationInputs.selectedCity}
                formatCreateLabel={(inputValue) => inputValue}
                styles={customSelectStyle}
              />
              <span className="underline-input"></span>
          ...
          <Col xs={12} md={4}>
            <div className="input-wrapper" style={{ width: "100%"  , marginTop:"8px"}}>
              ...
              <input
                style={{ height: " " }}
                type="TEXT"
                value={calulationInputs.locationCoordinates}
                onChange={(event) => {
                  setcalulationInputs({
                    ...calulationInputs,
                    locationCoordinates: event.target.value,
                  });
                }}
              />
              <span className="underline-input"></span>
        ...
        <Col xs={12} md={4}>
            ...
            <div className="input-wrapper" style={{ width: "100%"  , marginTop:"8px" }}>
              ...
              <select
                style={{ height: " " }}
                value={calulationInputs.statusOffer}
                onChange={(event) => {
                  setcalulationInputs({
                    ...calulationInputs,
                    statusOffer: event.target.value,
                  });
                }}
              >
                <option value="انتظار العقد">انتظار المراجعه</option>
           
                {selectStateShow?(
                  <>
                  <option value="المرفوض"> رفض</option>
                  <option value="انتظار المالك"> انتظار المالك</option>
                       <option value="انتظار النشر"> انتظار النشر</option>
                       <option value= "المنشور"> تم النشر</option>
                  </>
  ...
  <Row className="mt-3">
    {[
      ...
      <Col md={4} key={key} className="mb-4">
        <label>{label}</label>
        <div className="circle" style={{ width: "100%", height: "160px", position: "relative", backgroundColor: "#f0f0f0", borderRadius: "10px", overflow: "hidden" }}>
          {previewImages[key] ? (
            <img
            ...
            <FontAwesomeIcon
              icon={faImage}
              style={{ fontSize: "40px", color: "#aaa", position: "absolute", top: "40%", left: "40%" }}
            />
          )}
...
<Tab eventKey="images" title="صور العقار">
    ...
    {[
      ...
      <Col md={4} key={key} className="mb-4">
        ...
        <div
          ...
          style={{
            width: "100%",
            height: "250px", // تم زيادة الارتفاع ليكون مناسب
            position: "relative",
            backgroundColor: "#f0f0f0",
            borderRadius: "10px",
            overflow: "hidden",
          ...
          {previewImages[key] ? (
            <img
              ...
              style={{
                width: "100%",
                height: "100%",
                objectFit: "contain", // عرض الصورة بالكامل بدون قص
              }}
            />
          ) : (
            <FontAwesomeIcon
  ...
  <>
    ...
    {/* إظهار زر الحذف فقط إذا لم تكن الصورة الرئيسية */}
    {key !== "image" && (
      <button
  onClick={(e) => {
    e.stopPropagation();
...
<Tab eventKey="features" title="مميزات العقار">
 ...
 <Tab eventKey="contract" title="بيانات العقد">
      ...
      >
           ...
           <select
                      value={calulationInputs.typeOffers}
                      onChange={(event) => {
                        setcalulationInputs({
                          ...calulationInputs,
                          typeOffers: event.target.value,
                        });
                      }}
                    >
                         {/* <select name="bank"  value={calulationInputs.currentBank} onChange={e => changeUserFieldHandler(e)}> */}
                   
                      <option selected value="yesOffers"> مرخص</option>
                         <option value="noOffers">غير مرخص</option>
   ...
   {calulationInputs.typeOffers=="yesOffers"?(
          ...
          <Col xs={12} md={4}>
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <input
                style={{ height: " " }}
                name="contractNumber"
                value={calulationInputs.contractNumber}
             
                type="text"
                inputMode="numeric"
                maxLength={10}
                onChange={(e) => handleNumericInput(e, "contractNumber", 10)}
              />
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <select
                style={{ height: " " }}
                value={calulationInputs.ownerType}
                onChange={(event) => {
                  setcalulationInputs({
                    ...calulationInputs,
              ...
              >
               ...
               <option value="مالك"> مالك </option>
                ...
                <option value="شركة شخص واحد"> شركة شخص واحد</option>
                   <option value="شركة اكتر من شخص"> شركة اكتر من واحد</option>
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <input
                style={{ height: " " }}
                type="TEXT"
                value={calulationInputs.plotNumber}
                onChange={(event) => {
                  setcalulationInputs({
                    ...calulationInputs,
                    plotNumber: event.target.value,
                  });
                }}
              />
              <span className="underline-input"></span>
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <input
                style={{ height: " " }}
                type="TEXT"
                value={calulationInputs.planNumber}
                onChange={(event) => {
                  setcalulationInputs({
                    ...calulationInputs,
              ...
              <select
                style={{ height: " " }}
                value={calulationInputs.contractDuration}
                onChange={(event) => {
                  setcalulationInputs({
                    ...calulationInputs,
                    contractDuration: event.target.value,
                  });
                }}
              >
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <select
                style={{ height: " " }}
                value={calulationInputs.hasAgency}
                onChange={(event) => {
                  setcalulationInputs({
                    ...calulationInputs,
                    hasAgency: event.target.value,
                  });
                }}
              >
                
                  {calulationInputs.ownerType==="مالكان"||calulationInputs.ownerType==="شركة اكتر من شخص"?(<></>):( <option value="لا">لا</option>)}
          ...
          <Col xs={12} md={4}>
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <CreatableSelect
                ...
                value={ownerNameOptions.find(
                  (e) => e.value === calulationInputs.ownerName
                )}
...
{calulationInputs.ownerType==="مالك"||calulationInputs.ownerType==="مالكان"?(<></>):
...
(<>
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <CreatableSelect
                ...
                value={companyNameOptions.find(
                  (e) => e.value === calulationInputs.companyName
                )}
              ...
              <input
                style={{ height: " " }}
                name="price"
                value={calulationInputs.companyRegistry}
                type="text"
                inputMode="numeric"
                maxLength={10}
                onChange={(e) => handleNumericInput(e, "companyRegistry", 10)}
              />
              <span className="underline-input"></span>
            </div>  
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <input
                ...
                inputMode="numeric"
                maxLength={10}
                onChange={(e) => handleNumericInput(e, "ownerID", 10)}
              />
              <span className="underline-input"></span>
            </div>
          ...
          <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <input
                style={{ height: " " }}
                name="price"
                value={calulationInputs.ownerPhone}
                type="text"
                inputMode="numeric"
                maxLength={10}
                onChange={(e) => handleNumericInput(e, "ownerPhone", 10)}
              />
              <span className="underline-input"></span>
            </div>
  ...
  {/* <input
    name="birthDate"
    value={calulationInputs.ownerBirthDate}
    type="date"
    onChange={(e) => setcalulationInputs({
      ...calulationInputs,
      ownerBirthDate: e.target.value
    })}
    pattern="\d{2}-\d{2}-\d{4}"
  /> */}
{/* 
  ...
  <DatePicker
    label="اختر تاريخ"
    value={calulationInputs.ownerBirthDate ? dayjs(calulationInputs.ownerBirthDate) : null}
    onChange={(newValue) => {
      setcalulationInputs({
        ...calulationInputs,
        ownerBirthDate: newValue ? newValue.format("YYYY-MM-DD") : "",
      });
    }}
...
<div className="input-wrapper m-r-b" style={{ width: "100%" }}>
  <label>
    <FontAwesomeIcon
      icon={faCalendarAlt}
      style={{ marginLeft: "6px", color: "#888" }}
...
{/* <DatePicker
  selected={calulationInputs.ownerBirthDate}
  onChange={(date) =>
    setcalulationInputs({
      ...calulationInputs,
      ownerBirthDate: date
  ...
  placeholderText="اليوم - الشهر - السنة"
...
/> */}
    ...
    <LocalizationProvider dateAdapter={AdapterDateFns} adapterLocale={customAr}>
      <DatePicker
        slotProps={{
          textField: {
            placeholder: "dd-mm-yyyy", // 👈 يظهر Placeholder بدل Label
          },
        }}
        value={calulationInputs.ownerBirthDate}
        onChange={(newValue) =>
          setcalulationInputs({
            ...calulationInputs,
            ownerBirthDate: newValue,
          })
        }
        views={["year", "day"]}      // يبدأ باختيار السنة
        openTo="year"                // يفتح مباشرة على السنوات
        minDate={new Date("1925-01-01")}
        maxDate={new Date("2007-12-31")}
        defaultValue={new Date("1990-05-01")} // يفتح على مايو (5) 1990
      />
    </LocalizationProvider>
  ...
  {/* <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="ar">
    <DatePicker
      value={
        calulationInputs.ownerBirthDate
          ? dayjs(calulationInputs.ownerBirthDate)
          : null
      }
      onChange={(newValue) => {
        setcalulationInputs({
      ...
      slotProps={{
        textField: {
          ...
          fullWidth: true,
          size: "small",
          InputLabelProps: { shrink: false }, // 🔹 يمنع اللابل فوق
        },
      }}
    />
  </LocalizationProvider> */}
 ...
 { calulationInputs.hasAgency=="نعم"?(
<>
 <div className="input-wrapper" style={{ width: "100%" }}>
              <label>
                <FontAwesomeIcon
              ...
              <input
                style={{ height: " " }}
                name="price"
                value={calulationInputs.agencyNumber}
                type="text"
                inputMode="numeric"
                maxLength={10}
                onChange={(e) => handleNumericInput(e, "agencyNumber", 10)}
              />
              <span className="underline-input"></span>
            </div>
 <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <CreatableSelect
                placeholder="اختر أو اضف الموكل "
                options={agentNameOptions}
                value={agentNameOptions.find(
                  (e) => e.value === calulationInputs.agentName
                )}
            ...
            <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <input
                style={{ height: " " }}
                name="price"
                value={calulationInputs.agentID}
                type="text"
                inputMode="numeric"
                maxLength={10}
                onChange={(e) => handleNumericInput(e, "agentID", 10)}
              />
              <span className="underline-input"></span>
            </div>
          ...
          <div className="input-wrapper" style={{ width: "100%" }}>
              ...
              <input
                style={{ height: " " }}
                name="price"
                value={calulationInputs.agentPhone}
                type="text"
                inputMode="numeric"
                maxLength={10}
                onChange={(e) => handleNumericInput(e, "agentPhone", 10)}
              />
              <span className="underline-input"></span>
            </div>
...
<div className="input-wrapper m-r-b" style={{ width: "100%" }}>
  ...
  {/* <input
    name="birthDate"
    value={calulationInputs.agentBirthDate}
    type="date"
    onChange={(e) => setcalulationInputs({
      ...calulationInputs,
      agentBirthDate: e.target.value
    })}
    pattern="\d{2}-\d{2}-\d{4}"
  /> */}
...
{/* 
  ...
  <DatePicker
  selected={calulationInputs.agentBirthDate}
  onChange={(date) =>
    setcalulationInputs({
      ...calulationInputs,
      agentBirthDate: date
...
{/* <DatePicker
  ...
  minDate={new Date("1925-01-01")}
  maxDate={new Date("2007-12-31")}
  openToDate={new Date("1990-01-01")}   // يبدأ على 1990
/> */}
 ...
 {/* <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="ar">
    <DatePicker
      value={
        calulationInputs.agentBirthDate
          ? dayjs(calulationInputs.agentBirthDate)
          : null
      }
      onChange={(newValue) => {
        setcalulationInputs({
          ...calulationInputs,
          agentBirthDate: newValue ? newValue.format("YYYY-MM-DD") : "",
        });
      }}
      minDate={minAllowedDate}
      maxDate={maxAllowedDate}
...
{/* <LocalizationProvider dateAdapter={AdapterDayjs} adapterLocale="ar">
  <DatePicker
    label="اختر تاريخ"
    value={calulationInputs.agentBirthDate ? dayjs(calulationInputs.agentBirthDate) : null}
    onChange={(newValue) => {
      setcalulationInputs({
        ...calulationInputs,
        agentBirthDate: newValue ? newValue.format("YYYY-MM-DD") : "",
...
{/* 
  <Row className="mt-3">
  {[
    { key: "imageContract", label: "الصك" },
    { key: "imageAgent", label: "الوكاله" },
 ...
 */}
 ...
 <Row className="mt-3">
  {[
    ...
    .filter(({ key }) =>
      key === "imageAgent"
        ? calulationInputs.hasAgency == "نعم" // يظهر فقط إذا نعم
        : true
    )
    .map(({ key, label }) => (
      <Col md={4} key={key} className="mb-4">
...
{/* 
      ...
      {/* <div className="input-wrapper" style={{ width: "100%", marginTop: "20px" }}>
        <input value={`${datEAdd} ${timeAdd}`} readOnly style={{ width: "100%" }} />
        <span className="underline-input"></span>
      </div> */}
    </Container>
